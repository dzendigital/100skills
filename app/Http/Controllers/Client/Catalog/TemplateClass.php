...
{
    /**
     * Render itemlist template
     * 
     */
    public function item($list)
    {    
        $template = "";
        foreach ( (isset($list) && !empty($list) ? $list : array()) as $item) {
            $template .= "
            <div class='col-md-4 mt-4'>
                <div class='uk-panel p-3 border'>
                    ...
                    <div class='mt-3 mt-md-0'>
                        ...
                        <a href='#' class='btn border-btn-dark btn-block mt-4 m-none'>Посмотреть курс</a>
                    </div>
                </div>
            </div>
            ";
     ...
     * 
     */
    public function pagination($offset, $limit, $query, $paginator)
    {    
        # Page number
        $pageno = $offset + 1;
        # Perpage count
        ...
        $elements = $paginator->getUrlRange(1, $pages);
        foreach ( $elements as $page => $element ){
            if ( $page == $pageno){
                $template .= "<li class='uk-active' aria-current='page'><span>{$page}</span></li>";
                continue;
            }
            $template .= "<li><a href='{$element}'>{$page}</a></li>";
        }
        ...
        if ( $paginator->hasMorePages() ){
            ...
            $template .= "
                <li>
                    <a onclick='event.preventDefault();'><span uk-pagination-next></span></a>
                </li>
            ";
        }
        $template = "
